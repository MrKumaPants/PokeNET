...
public class JsonAssetLoader<T> : IAssetLoader<T> where T : class
{
    ...
    public JsonAssetLoader(ILogger<JsonAssetLoader<T>> logger, long streamingThreshold = 1_048_576)
    {
        ...
        _logger.LogDebug("JsonAssetLoader<{TypeName}> initialized with streaming threshold: {Threshold} bytes",
            typeof(T).Name, _streamingThreshold);
    }
    ...
    public T Load(string path)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully loaded JSON asset: {Path} as {TypeName}",
                path, typeof(T).Name);
        ...
        catch (JsonException jsonEx)
        {
            ...
            {
                errorMessage += $" at line {jsonEx.LineNumber}, position {jsonEx.BytePositionInLine}";
            }
    ...
    public bool CanHandle(string extension)
    {
        ...
        _logger.LogTrace("CanHandle check for extension '{Extension}': {Result}",
            extension, canHandle);
    ...
    private T LoadWithStreaming(string path)
    {
        ...
        if (asset == null)
        {
            throw new AssetLoadException(path, $"JSON streaming deserialization returned null for: {path}");
        }
    ...
    private void ValidateTypeCompatibility(JsonElement element, string path)
    {
        ...
        else if (targetType.IsClass)
        {
            ...
            {
                _logger.LogWarning("Type compatibility warning: Expected JSON object for class type {TypeName}, but found {ValueKind}",
                    targetType.Name, element.ValueKind);
            }
        }
    }
