...
public class CalculateStatsCommand : CliCommand<CalculateStatsCommand.Settings>
{
    ...
    public CalculateStatsCommand(CliContext context) : base(context) { }
    ...
    protected override async Task ExecuteCommandAsync(CommandContext context, Settings settings)
    {
        await AnsiConsole.Status()
            .StartAsync("Loading species data...", async ctx =>
            {
                ...
                {
                    AnsiConsole.MarkupLine("[red]Invalid IVs or EVs format. Use: HP,Atk,Def,SpA,SpD,Spe[/]");
                    return;
                }
    ...
    private static void DisplayStats(SpeciesData species, PokemonStats stats, int level, Nature nature)
    {
        // Header
        ...
        AddStatRow(table, "Attack", species.BaseStats.Attack, stats.IV_Attack, stats.EV_Attack, stats.Attack);
        AddStatRow(table, "Defense", species.BaseStats.Defense, stats.IV_Defense, stats.EV_Defense, stats.Defense);
        AddStatRow(table, "Sp. Attack", species.BaseStats.SpecialAttack, stats.IV_SpAttack, stats.EV_SpAttack, stats.SpAttack);
        AddStatRow(table, "Sp. Defense", species.BaseStats.SpecialDefense, stats.IV_SpDefense, stats.EV_SpDefense, stats.SpDefense);
        AddStatRow(table, "Speed", species.BaseStats.Speed, stats.IV_Speed, stats.EV_Speed, stats.Speed);

        AnsiConsole.Write(table);

        var totalEVs = stats.EV_HP + stats.EV_Attack + stats.EV_Defense +
                       stats.EV_SpAttack + stats.EV_SpDefense + stats.EV_Speed;
        AnsiConsole.MarkupLine($"\n[grey]Total EVs: {totalEVs}/510[/]");
    }

    private static void AddStatRow(Table table, string name, int baseValue, int iv, int ev, int final)
    {
        table.AddRow(
            name,
            baseValue.ToString(),
